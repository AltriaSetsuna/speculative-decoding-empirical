    ...
    if is_online:
        if (
            ...
            if args.target_model_backend == "sglang":
                ...
                # SGLang/vLLM 真正识别的参数名通常不带 sglang_ 前缀
                # 我们同时设置两个常见的参数名，确保万无一失
                target_model_kwargs["mem_fraction_static"] = 0.4    
                target_model_kwargs["max_running_requests"] =1 
                target_model_kwargs["context_length"] = 10000
                target_model_kwargs["max_total_tokens"] = 10000

      
                # 打印一下，确认参数真的改进去了
                print(f"[DEBUG] SGLang kwargs fixed: {target_model_kwargs}")
                # -----------------------------------------------
        ...
        print(f"[DEBUG] offline Loading target head from {args.target_model_path}")
...
def record_metrcs(
    ...
    if dist.is_initialized() and dist.get_rank() == 0:
        try:
            wandb.log(logdict, step=global_step)
...
def main():
    ...
    if dist.get_rank() == 0:
        wandb.init(
            project="Eagle3-Llama3-70B",  # 你可以在这里修改项目名称
            name=f"offline-train-{int(time.time())}", # 自动生成唯一的 Run Name
            config=vars(args) # 把所有训练参数也记录上去，方便以后查看
        )
    # ===============================
    ...
    # # 【新增】开启梯度检查点
    # draft_model.gradient_checkpointing_enable()  # <--- 加这一行！
    ...
    # [新增] 训练结束后的强制保存
    # ================================================
    print_on_rank0("Training finished. Saving the final model...")
