#!/usr/bin/env python3
"""
Run vLLM deployment scripts one by one, execute the matching CanItEdit script,
...
with:
  CanItEdit/scripts/<model>/<method>.sh

Examples:
  Qwen3-32B.sh/eagle3_RedHatAI.sh -> CanItEdit/scripts/Qwen3-32B/eagle3.sh
  L31-70B-I.sh/suiffix.sh         -> CanItEdit/scripts/L31-70B-I/suffix.sh
"""
...
CANITEDIT_DIR = ROOT / "CanItEdit"
CANITEDIT_SCRIPTS_DIR = CANITEDIT_DIR / "scripts"
RESULTS_DIR = ROOT / "vllm" / "canitedit" / "results"
    ...
    for model_dir in sorted(VLLM_SCRIPTS_DIR.iterdir()):
        ...
        for vllm_script in sorted(model_dir.glob("*.sh")):
            ...
            if methods and method not in methods:
                continue
            canitedit_script = CANITEDIT_SCRIPTS_DIR / model / f"{method}.sh"
            if canitedit_script.exists():
                jobs.append(Job(model, method, vllm_script, canitedit_script))
            else:
                print(
                    f"[skip] no CanItEdit script for {model}/{method}: "
                    f"{canitedit_script}",
                    flush=True,
                )
    return jobs
...
def http_get_text(url: str, timeout: float = 5.0) -> str | None:
    try:
        req = Request(url, headers={"User-Agent": "canitedit-vllm-sweep/1.0"})
...
def canitedit_output_dir(job: Job) -> Path | None:
    target_model = parse_target_model(job.canitedit_script)
    if not target_model:
        return None
    return CANITEDIT_DIR / "out" / target_model.rsplit("/", 1)[-1] / job.method


def open_json_result(path: Path) -> dict[str, object] | None:
    try:
        if path.suffix == ".gz":
...
def parse_canitedit_file(path: Path) -> dict[str, object] | None:
    data = open_json_result(path)
    if data is None:
        return None
...
def parse_canitedit_summary(output_dir: Path | None) -> dict[str, object]:
    if output_dir is None or not output_dir.exists():
        return {}
    paths = sorted(output_dir.glob("*.results.json")) + sorted(
        output_dir.glob("*.results.json.gz")
    )
    results = [parse_canitedit_file(path) for path in paths]
    results = [result for result in results if result is not None]
    if not results:
        return {}
    total_completions = sum(int(result["n"]) for result in results)
    total_correct = sum(int(result["c"]) for result in results)
    return {
        "pass@1": sum(float(result["pass@1"]) for result in results) / len(results),
        "pass@10": sum(float(result["pass@10"]) for result in results) / len(results),
        "pass@100": sum(float(result["pass@100"]) for result in results) / len(results),
        "canitedit_num_problems": len(results),
        "canitedit_min_completions": min(int(result["n"]) for result in results),
        "canitedit_max_completions": max(int(result["n"]) for result in results),
        "canitedit_total_completions": total_completions,
        "canitedit_total_correct": total_correct,
    }


def docker_image_exists(image: str, docker_bin: str) -> bool:
    proc = subprocess.run(
...
def ensure_docker_image(image: str, docker_bin: str) -> None:
    if docker_image_exists(image, docker_bin):
        return
    mirror_image = f"hub.mirrorify.net/{image}"
    subprocess.run([docker_bin, "pull", mirror_image], check=True)
    subprocess.run([docker_bin, "tag", mirror_image, image], check=True)


def run_canitedit_evaluation(
    ...
    if output_dir is None:
        return None, 0.0, "could not parse TARGET_MODEL for CanItEdit output dir"
    if not output_dir.exists():
        return None, 0.0, f"CanItEdit output dir does not exist: {output_dir}"
    ensure_docker_image(image, docker_bin)
    command = [image, "--dir", "/inputs", "--output-dir", "/outputs"]
    if not coverage:
        command = ["--entrypoint", "python3", image, "main.py", "--dir", "/inputs", "--output-dir", "/outputs"]
    start = time.time()
    with log_path.open("w") as log:
        proc = subprocess.Popen(
            [
                ...
                "none",
                "--volume",
                f"{output_dir.resolve()}:/inputs:ro",
                "--volume",
        ...
        try:
            ...
            try:
                ...
                proc.kill()
                exit_code = proc.wait()
            return exit_code, time.time() - start, f"timeout after {timeout_s}s"


def maybe_clear_canitedit_output(job: Job, fresh_output: bool) -> str | None:
    if not fresh_output:
        return None
    output_dir = canitedit_output_dir(job)
    if output_dir is None:
...
def completed_runtime_seconds(row: dict[str, object] | None) -> float | None:
    if row is None or not is_completed(row):
        return None
    try:
        return float(row["canitedit_seconds"])
    except (TypeError, ValueError):
        return None
...
def is_completed(row: dict[str, object]) -> bool:
    if row.get("status") != "ok":
        return False
    if row.get("canitedit_exit_code") != 0:
        return False
    try:
...
def append_csv(path: Path, row: dict[str, object]) -> None:
    fieldnames = [
        ...
        "vllm_exit_code",
        "canitedit_exit_code",
        "canitedit_eval_exit_code",
        "startup_seconds",
        "canitedit_seconds",
        "canitedit_eval_seconds",
        "grafana_reachable",
        "pass@1",
        "pass@10",
        "pass@100",
        "canitedit_num_problems",
        "canitedit_min_completions",
        "canitedit_max_completions",
        "canitedit_total_completions",
        "canitedit_total_correct",
        "tps",
        "tps_source",
        "mean_accept_length",
...
def run_job(job: Job, args: argparse.Namespace, summary_csv: Path, summary_jsonl: Path) -> dict[str, object]:
    ...
    canitedit_log = result_dir / "canitedit.log"
    canitedit_eval_log = result_dir / "canitedit_eval.log"
    ...
    row: dict[str, object] = {
        ...
        "canitedit_script": str(job.canitedit_script),
        "vllm_log": str(vllm_log),
        "canitedit_log": str(canitedit_log),
        "canitedit_eval_log": str(canitedit_eval_log),
        "metrics_snapshot": str(metrics_snapshot),
        "canitedit_output_dir": str(output_dir) if output_dir else "",
        "error": "",
    }
    ...
    if not args.force:
        ...
        if completed:
            completed.update(parse_canitedit_summary(output_dir))
    ...
    cleanup_note = maybe_clear_canitedit_output(job, args.fresh_output)
    if cleanup_note:
    ...
    try:
        ...
        print("[ok] vLLM healthy; running CanItEdit", flush=True)
        row["grafana_reachable"] = bool(http_get_text(GRAFANA_URL, timeout=3.0))

        canitedit_exit, canitedit_seconds, canitedit_error = run_command(
            job.canitedit_script,
            CANITEDIT_DIR,
            canitedit_log,
            args.canitedit_timeout,
            child_env,
        )
        row["canitedit_exit_code"] = canitedit_exit
        row["canitedit_seconds"] = round(canitedit_seconds, 3)
        if canitedit_error:
            row["error"] = canitedit_error

        canitedit_eval_exit, canitedit_eval_seconds, canitedit_eval_error = (
            run_canitedit_evaluation(
                output_dir,
                canitedit_eval_log,
                args.canitedit_eval_timeout,
                args.canitedit_eval_image,
                args.docker_bin,
                args.canitedit_eval_coverage,
            )
        )
        row["canitedit_eval_exit_code"] = canitedit_eval_exit
        row["canitedit_eval_seconds"] = round(canitedit_eval_seconds, 3)
        if canitedit_eval_error:
            row["error"] = (
                str(row.get("error") or "") + f"; CanItEdit eval: {canitedit_eval_error}"
            ).strip("; ")
        row.update(parse_canitedit_summary(output_dir))
        ...
        row["status"] = (
            "ok"
            if canitedit_exit == 0 and canitedit_eval_exit == 0
            else "canitedit_failed"
        )
        if canitedit_exit == 0 and canitedit_eval_exit != 0:
            row["status"] = "canitedit_eval_failed"
        if row["status"] == "ok" and row.get("tps") is None:
            row["status"] = "no_metrics"
            row["error"] = (
                str(row.get("error") or "")
                + "; CanItEdit finished but vLLM token metrics were empty. "
                "Most likely the output dir already existed and CanItEdit skipped requests."
            ).strip("; ")
    ...
    except Exception as exc:  # Keep the sweep moving to the next script.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run vLLM + CanItEdit benchmark sweep.")
    ...
    parser.add_argument("--canitedit-timeout", type=float, default=None)
    parser.add_argument("--canitedit-eval-timeout", type=float, default=None)
    parser.add_argument("--canitedit-eval-image", default="edit-eval:latest")
    parser.add_argument(
        "--canitedit-eval-coverage",
        action="store_true",
        help="also compute CanItEdit coverage/ExcessCode fields; pass@1 does not need this",
    )
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="rerun jobs even if vllm/results/<model>/<method>/result.json is complete",
    )
    parser.add_argument(
        "--fresh-output",
        action="store_true",
        help="remove the matching CanItEdit out/<model>/<method> directory before each job",
    )
    parser.add_argument(
    ...
    summary_csv = RESULTS_DIR / "canitedit_vllm_sweep.csv"
    summary_jsonl = RESULTS_DIR / "canitedit_vllm_sweep.jsonl"
    summary_md = RESULTS_DIR / "canitedit_vllm_sweep.md"
    ...
    for job in jobs:
        print(f"  {job.model}/{job.method}: {job.vllm_script.name} -> {job.canitedit_script}")
